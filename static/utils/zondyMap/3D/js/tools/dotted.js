    ...
    //鼠标移动时的那条线 
	var moveLine_first = null;
	var moveLine_second = null;
    

    // 场景搭建完成时触发 
    function viewerInit(){
        points = viewer.entities.add(new Cesium.Entity());
        polylines = viewer.entities.add(new Cesium.Entity());
        Polygons_Line = viewer.entities.add(new Cesium.Entity());
        Polygon_Lines_remove = viewer.entities.add(new Cesium.Entity());
        canvas.onclick = function () {
            canvas.focus();
        };
        moveLine_first =moveLine("moveLine_first");
        moveLine_second =moveLine("moveLine_second");
    ...
    function yidong(cartesian,curMovementLon,curMovementLat,height){
        if (isPolyline || isPolygon_line || isRound ||isArc) {           
            if (cartesian && isStartDraw) {
                ...
                if (Cesium.defined(viewer.entities.getById("moveLine_first"))) {
            ...
            if (cartesian) {
                ...
                if (!isStartDraw){
                    ...
                    if (isPolyline || isPolygon_line )
                    {
                        ...
                        if (Cesium.defined(viewer.entities.getById("moveLine_second"))) {
    ...
    function clearDrawData() {
        if (polylines) {
            polylines.entityCollection.removeAll();
            viewerInit();
